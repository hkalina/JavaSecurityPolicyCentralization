...
V této podkapitole bude vysvětlen způsob vytvoření vlastního Security manageru za účelem stanovení bezpečnostní politiky. Toto je starší způsob stanovení bezpečnostní politiky uplatňované JVM, v současných verzích Javy je možné bezpečnostní politku stanovit také jednodušším způsobem, ke kterému se dostamene později. Implementace Security managera ale dovoluje založit rozhodování o povolení nebo nepovolení žádané operace na takřka jakémkoli algoritmu, jak v této kapitole ukážeme.

{\tt SecurityManager} (přesněji {\tt java.lang.SecurityManager}) tvoří rozhraní, skrze které je Java API schopno zjistit, zdali je operace požadovaná uživatelskou aplikací povolena. Algoritmus který Java API používá při provádění potenciálně nebezpečné operace je následující: \cite[4.1.1]{oaks}
...
Zkusíme-li spustit sebejednodušší program pod tímto Security managerem, uvidíme že množství kontrolovaných operací je opravdu velké a tvorba Security manageru, povolujícího operace vyžadované jednotlivými programy těmto konkrétním programům, by byla možná, ale zbytečně složitá.
...
Nyní se pozastavíme nad tématem zavádění tříd do paměti. Ačkoli se na první pohled může zdát, že toto téma s bezpečností příliš nesouvisí, opak je pravdou. Jen a pouze zavaděč třídy, který třídu do paměti zavádí totiž může určit původ třídy, od kterého se oprávnění zdrojového kódu třídy odvozuje.
...
Ochranná doména ({\tt ProtectionDomain}) je uskupení zdrojových kódů a oprávnění, které jsou tomuto kódu udělena. Speciálním případem jsou třídy zavedené interním zavaděčem (\ref{interniZavadec}), které spadají do systémové ochrané domény (jejich reference na ochranou doménu je {\tt null}) a mají tak vždy povoleno vše. \cite[5.4]{oaks}
...
Access controller je mechanismus, který standardní Security manager (od Javy verze 2) využívá pro rozhodnutí, zda operaci požadovanou uživatelským programem povolit či nikoliv. Aby tedy byla aplikována omezení stanovená Access controller, musí být JVM spuštěna se standardním Security managerem. \cite[5]{oaks}
