...
Hodnota proměnné je inicializována při prvním pokusu o získání její hodnoty metodou {\tt get()}, nebyla-li dříve nastavena voláním metody {\tt set()}. Hodnota na kterou je proměnná incializována je určena voláním metody svého objektu, {\tt initialValue()}. Tato metoda vrací implicitně {\tt null}. Jiné implicitní hodnoty lze dosáhnout přepsáním této metody vlastní implementací. Právě přepisem této metody je stanovena i implicitní hodnota proměnné {\tt CHECKING} v mé záplatě (Kód \ref{patchSM}). \cite{refThreadLocal}
...
Hlavní překážkou v aplikaci bezpečnostní politiky ze souboru bezpečnostní politiky lze nalézt v zavaděči tříd modulů WildFly ({\tt org.jboss.modules.ModuleClassLoader}). Tento zavaděč zajišťuje načtení, a tím také přidělení oprávnění, tříd modulů WildFly. Mezi ty se však v tomto případě počítají také třídy balíčků uživatelských aplikací.

Součástí práce zavaděče tříd je přidělovat třídám, které načítají, ochrannou doménu a tím také jejich oprávnění. Zatímco Zavaděč tříd z URL (popsaný v kapitole \ref{URLClassLoader}) přiděluje třídám oprávnění podle objektu bezpečnostní politiky (instance (pod)třídy {\tt java.lang.Policy}), Zavaděč tříd modulů WildFly přiděluje třídám oprávnění na základě oprávnění modulu, z něhož jsou načítány. Ta jsou v případě modulu z adresáře {\tt modules} neomezená (?) a v případě balíčku uživatelské aplikace určena konfiguračním souborem {\tt permission.xml} určeného specifikací Java EE. \cite{javaEEspec}
